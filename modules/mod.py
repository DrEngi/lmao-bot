...
class Mod:
    ...
    async def cmd_purge(self, ctx, *, arg=""):  # Allows the deletion of messages
        if perms.get_perms(ctx.message).manage_messages:
            try:
                if int(arg) > 100:
    ...
    async def cmd_mute(self, ctx, *, arg=""):
        ...
        if perms.get_perms(ctx.message).manage_channels:
            try:
                if len(ctx.message.mentions) == 1:
    ...
    async def cmd_kick(self, ctx, *, arg=""):
        if perms.get_perms(ctx.message).kick_members:
            try:
                if len(ctx.message.mentions) == 1:
                    if ctx.message.mentions[0] == self.bot.user:
    ...
    async def cmd_ban(self, ctx, *, arg=""):
        if perms.get_perms(ctx.message).ban_members:
            try:
                if len(ctx.message.mentions) == 1:
                    if ctx.message.mentions[0] == self.bot.user:
